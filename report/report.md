...
### A multi-method study, a controlled within-family scale follow-up, and a framing test
...
suggesting order imbalance can obscure cross-method agreement. Study 2b
...
three studies: a multi-method convergence study, a within-family scale
comparison (with a provider-controlled replication, Study 2b), and a test of
whether the elicitation prompt's own framing drives the artefact.
...
scale, data, post-training and serving stack simultaneously. **Study 2b**
substantially reduces these confounds by holding model family, upstream inference
provider, prompts, sampling, task set and protocol fixed while comparing two model
scales. (The original Study 2 did not pin the upstream provider; see §6.4.)
...
the lowest content signal (0.079) alongside the highest position
...
welfare, phenomenology or genuine internal preference. All three studies measure
...
elicitation procedures across three model families, then ran a controlled
within-family scale comparison on two more, and finally tested whether our own
prompt framing drives the effect.
...
(Llama), and was not detectable in the third (Qwen). The model differences
coincided with large differences in order-invariant signal. In one model the
standard pairwise procedure recovered nothing but display order, while still
passing validity controls; in another, equal-weighting the display orders raised
an apparently null cross-method correlation to +0.524, so order imbalance can
obscure agreement as well as fabricate it.

A follow-up with pre-specified hypotheses (Study 2b), holding family and upstream
provider fixed, found the larger model *more* position-dominated, rejecting the
capability explanation we had proposed. Removing the explicit indifference cue
left GPT-OSS 120B extremely position-dominated, while the 20B model showed a
suggestive reduction in positional susceptibility.
